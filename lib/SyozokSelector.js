...
export default class SyozokSelector extends React.Component
{
	...
	render()
	{
		...
		const selected = options.findIndex(({ value }) => value === syozok);
	...
	componentDidMount()
	{
		...
		this.syozokInput.input.on('wheel', this.wheel);
		this.syozokInput.barsIcon.on('click', this.homeIconClick);
	}
	componentWillUnmount()
	{
		this.syozokInput.input.off('keydown', this.keydown);
		this.syozokInput.input.off('wheel', this.wheel);
		this.syozokInput.barsIcon.off('click', this.homeIconClick);
	}
	keydown = e =>
	{
		this.options.keydown(e);
	}
	optionsSelected = (index, option, method) =>
	{
		if (!option) return;
